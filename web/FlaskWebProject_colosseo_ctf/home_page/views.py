...
def publish(id,question_id):
    ...
    if request.method == "POST":
        ...
        old_writeup = db_session.query(Article).filter(Article.question_id == question_id and Article.user_name == user.name).first()
        if not old_writeup:
            # 获取博客内容
...
#writeup列表
@home_page.route("/writeup_list/<id>/<question_id>/<question_name>")
@login_required
def writeup_list(id,question_id,question_name):
    user = db_session.query(User).filter(User.sequence == id).first()
    Writeups = db_session.query(Article).filter(Article.question_id == question_id).all()
    question = db_session.query(Question).filter(Question.sequence == question_id).first()

    return render_template('/homepage_files/writeup_list.html',id=id,user=user,Writeups=Writeups,question_name=question.name)
